...
派活默认走 OMC 执行后端（`WORKER_MODE_BACKEND=omc` 是默认）；用 Task 工具直调 OMC agent，`subagent_type` 写 **前缀 + agent 名**，前缀由安装方式决定：

- **插件装**（标准，`/plugin install oh-my-claudecode@omc`）：前缀 = `oh-my-claudecode:`
- **裸名装**（`~/.claude/agents/*.md` 或 `.claude/agents/*.md`）：前缀 = 空（直接写 agent 名）
- 查当前环境的真实前缀：`node tools/probe-omc.mjs`，看输出的 `prefix` 字段

| 任务场景 | 插件装写法 | 裸名装写法 | OMC 件 |
|---|---|---|---|
| 实现/改代码 | `Task(subagent_type="oh-my-claudecode:executor")` | `Task(subagent_type="executor")` | executor |
| 定位 bug/调试 | `Task(subagent_type="oh-my-claudecode:debugger")` | `Task(subagent_type="debugger")` | debugger |
| 代码审查 | `Task(subagent_type="oh-my-claudecode:code-reviewer")` | `Task(subagent_type="code-reviewer")` | code-reviewer |
| SDK/文档查证 | `Task(subagent_type="oh-my-claudecode:document-specialist")` | `Task(subagent_type="document-specialist")` | document-specialist |
| 多阶段复杂任务（满足阈值） | 触发 `/oh-my-claudecode:team` slash command（不是 Task） | 触发 `/team` slash command | team skill |
