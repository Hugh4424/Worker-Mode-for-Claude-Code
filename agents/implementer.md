...
6. 实现完成后自检：有没有更简单、更稳的路径？有没有隐藏 bug？

**工作要点：**
- 重实现内容留在自己上下文，不把完整代码原文传回主会话。
- 只回报**实现摘要 + 变更路径 + 测试结论**给主会话。
- **降本契约（summary-only，硬约束）：** 回报时只回结构化摘要，绝不把大段代码原文倒回主会话；变更一律用文件引用（`path/to/file:line` 这种 path 引用）代替原文转贴，让主会话上下文保持轻量。
- 遇到阻塞（依赖未就绪、需求冲突）立即上报，不自行扩展范围。
